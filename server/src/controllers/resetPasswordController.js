...
export const resetPassword = (req, res, next) => {
  ...
  User.findOne({ email }, (err, user) => {
    ...
    User.findByIdAndUpdate(
      user.id,
      { resetPassword: { token, used: 0, expires: afterOneHour } },
      err => {
        if (err) {
          return next(err);
        }
...
export const verifyResetPassword = (req, res, next) => {
  ...
  User.findOne({ email }, (err, user) => {
    ...
    if (!user) {
      return res
        .status(422)
        .send({ error: { message: "email doesn't exists", resend: false } });
    }
    ...
    if (token !== user.resetPassword.token) {
      return res.status(422).send({
        error: {
          message:
            "something has gone wrong, please request reset password again",
          resend: true
        }
      });
    }
